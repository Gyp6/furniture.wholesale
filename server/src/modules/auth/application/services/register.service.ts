...
export class RegisterService {
  ...
  afterSignUp(context: AuthHookContext) {
    const body = context.body as RegisterRequest;
    const user = (context.context as any)?.newSession?.user as {
      id: string;
      email: string;
    };
  ...
  private async handleBackgroundRegisterTask(
    ...
    await this.prismaService.$transaction(async tx => {
      let company = await this.companyRepository.findByTaxCode(
        body.taxCode,
        tx,
      );

      if (!company) {
        const baseCode = generateAbbreviation(body.companyName);

        let uniqueCode = baseCode;
        let counter = 1;

        // ЦИКЛ ПЕРЕВІРКИ НА УНІКАЛЬНІСТЬ
        while (true) {
          // Викликаємо репозиторій і обов'язково передаємо контекст tx!
          const existingCompany =
            await this.companyRepository.findByAbbreviation(uniqueCode, tx);

          // Якщо абревіатура вільна — виходимо з циклу
          if (!existingCompany) {
            break;
          }

          // Якщо код зайнятий, додаємо цифру: HC -> HC1 -> HC2
          uniqueCode = `${baseCode.substring(0, 2)}${counter}`;
          counter++;
        }

        const abbreviation = uniqueCode;
