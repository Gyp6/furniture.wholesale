...
import { CATEGORY_REPOSITORY, type ICategoryRepository } from '@catalog/domain/contracts';
...
describe('CategoryService', () => {
  ...
  const mockCategoryRepository = {
    ...
    create: mock((name: string) => Promise.resolve(null as any)),
  };
  ...
  describe('findAll', () => {
    it('should return all categories mapped to response', async () => {
      ...
      mockCategoryRepository.findAll.mockImplementation(() => Promise.resolve(mockCategories));
  ...
  describe('findById', () => {
    it('should return a category mapped to response if found', async () => {
      const mockCategory = new Category('123', 'Office Chairs', 'office-chairs');
      mockCategoryRepository.findById.mockImplementation((id) => Promise.resolve(mockCategory));
    ...
    it('should throw NotFoundException if category not found by id', async () => {
      mockCategoryRepository.findById.mockImplementation((id) => Promise.resolve(null));
  ...
  describe('findBySlug', () => {
    it('should return a category mapped to response if found by slug', async () => {
      const mockCategory = new Category('456', 'Kitchen Tables', 'kitchen-tables');
      mockCategoryRepository.findBySlug.mockImplementation((slug) => Promise.resolve(mockCategory));
    ...
    it('should throw NotFoundException if category not found by slug', async () => {
      mockCategoryRepository.findBySlug.mockImplementation((slug) => Promise.resolve(null));
  ...
  describe('create', () => {
    it('should create a new category and return it mapped to response', async () => {
      ...
      mockCategoryRepository.create.mockImplementation((name) => Promise.resolve(mockCategory));
