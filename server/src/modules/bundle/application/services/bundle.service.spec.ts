...
import { SpaceService, ProductService } from '@catalog/application/services';
import { BUNDLE_REPOSITORY, type IBundleRepository } from '@bundle/domain/contracts';
import { Bundle, BundleItem } from '@bundle/domain/entities';
import { Space, Product } from '@catalog/domain/entities';
import { BundleNotFoundException, BundleNotSharedError, CannotForkSupplierBundleError } from '@bundle/domain/exceptions';
...
describe('BundleService', () => {
  ...
  const mockSpace = new Space('space-id', 'Living Room', 'living-room', new Date(), new Date());
  ...
  const createMockBundle = (id = 'bundle-id', type: 'SUPPLIER' | 'USER' = 'USER', userId = 'user-id') => {
    return new Bundle(
      id,
      type,
      1,
  ...
  const mockBundleRepository = {
    findAllByUserId: mock((userId: string, type: any) => Promise.resolve([])),
    findAllSuppliers: mock((params: any) => Promise.resolve([])),
    findById: mock((id: string) => Promise.resolve(null as any)),
    findByShareToken: mock((token: string) => Promise.resolve(null as any)),
    create: mock((entity: Bundle) => Promise.resolve(entity)),
    update: mock((id: string, dto: any) => Promise.resolve(null as any)),
    findRaw: mock((id: string) => Promise.resolve(null as any)),
    addItem: mock((id: string, item: BundleItem) => Promise.resolve(null as any)),
    removeItem: mock((id: string) => Promise.resolve()),
  ...
  describe('findAllByUser', () => {
    it('should return mapped bundles', async () => {
      ...
      mockBundleRepository.findAllByUserId.mockImplementation(() => Promise.resolve([mockBundle]));
  ...
  describe('findAllSuppliers', () => {
    it('should return mapped bundles for supplier parameters', async () => {
      const mockBundle = createMockBundle('bundle-supplier', 'SUPPLIER', 'supplier-id');
      mockBundleRepository.findAllSuppliers.mockImplementation(() => Promise.resolve([mockBundle]));

      const result = await service.findAllSuppliers({ supplierId: 'supplier-id' });
      expect(result).toHaveLength(1);
  ...
  describe('findById', () => {
    it('should return mapped bundle if found', async () => {
      const mockBundle = createMockBundle();
      mockBundleRepository.findById.mockImplementation(() => Promise.resolve(mockBundle));
    ...
    it('should throw BundleNotFoundException if not found', async () => {
      mockBundleRepository.findById.mockImplementation(() => Promise.resolve(null));
      expect(service.findById('bundle-id')).rejects.toThrow(BundleNotFoundException);
    });
  });
  ...
  describe('findByShareToken', () => {
    it('should return mapped bundle if found', async () => {
      const mockBundle = createMockBundle();
      mockBundleRepository.findByShareToken.mockImplementation(() => Promise.resolve(mockBundle));
    ...
    it('should throw BundleNotSharedError if not found', async () => {
      mockBundleRepository.findByShareToken.mockImplementation(() => Promise.resolve(null));
      expect(service.findByShareToken('token')).rejects.toThrow(BundleNotSharedError);
    });
  });

  describe('create', () => {
    it('should create bundle if role matches bundle type', async () => {
      mockSpaceService.findById.mockImplementation(() => Promise.resolve(mockSpace));
      const user = { id: 'user-id', role: 'SUPPLIER' };
      const dto = { name: 'New Bundle', bundleType: 'SUPPLIER' as const, spaceTypeId: 'space-id' };
    ...
    it('should throw ForbiddenException if buyer tries to create supplier bundle', async () => {
      ...
      expect(service.create(user as any, dto)).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should throw NotFoundException if space type is not found', async () => {
      ...
      const user = { id: 'user-id', role: 'SUPPLIER' };
      const dto = { name: 'New Bundle', bundleType: 'USER' as const, spaceTypeId: 'space-id' };
  ...
  describe('update', () => {
    it('should update bundle if allowed', async () => {
      ...
      mockBundleRepository.findRaw.mockImplementation(() => Promise.resolve(raw));
      const mockBundle = createMockBundle();
      mockBundleRepository.update.mockImplementation(() => Promise.resolve(mockBundle));

      const result = await service.update('bundle-id', { name: 'Updated' }, mockAbility as any);
      expect(result.id).toBe('bundle-id');
    });
    ...
    it('should throw ForbiddenException if update not allowed', async () => {
      ...
      expect(service.update('bundle-id', { name: 'Updated' }, mockAbility as any)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('share & unshare', () => {
    it('should enable sharing if owned', async () => {
      const mockBundle = createMockBundle();
      mockBundleRepository.findById.mockImplementation(() => Promise.resolve(mockBundle));
      mockBundleRepository.update.mockImplementation((id, data) => Promise.resolve({ ...mockBundle, ...data }));
    ...
    it('should disable sharing if owned', async () => {
      ...
      const result = await service.unshare('bundle-id', { id: 'user-id' } as any);
  ...
  describe('fork', () => {
    ...
    it('should throw error if bundle is a SUPPLIER bundle', async () => {
      ...
      expect(service.fork('bundle-id', { id: 'owner-2' } as any)).rejects.toThrow(CannotForkSupplierBundleError);
    });
  });
  ...
  describe('addItem', () => {
    it('should add a product item to a user bundle successfully', async () => {
      const mockBundle = createMockBundle();
      mockBundleRepository.findById.mockImplementation(() => Promise.resolve(mockBundle));
      mockProductService.findById.mockImplementation(() => Promise.resolve(mockProduct as any));

      const updatedBundle = createMockBundle();
      mockBundleRepository.addItem.mockImplementation(() => Promise.resolve(updatedBundle));
    ...
    it('should throw NotFoundException if product is not found', async () => {
      const mockBundle = createMockBundle();
      mockBundleRepository.findById.mockImplementation(() => Promise.resolve(mockBundle));
      mockProductService.findById.mockImplementation(() => Promise.resolve(null));
  ...
  describe('removeItem', () => {
    it('should remove item successfully if owned', async () => {
      ...
      await service.removeItem('bundle-id', 'item-id', { id: 'user-id' } as any);
