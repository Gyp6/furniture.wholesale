...
export function BundleCard({ name, vendor, description, price, itemsCount, images, onAdd }: TBundleCardProps) {
  return (
    <Card className="flex flex-row ring-0 shadow-[0_8px_40px_rgba(0,0,0,0.08)] border border-neutral-100 gap-0 p-5 rounded-[40px] h-[340px]">
      ...
      <div className="flex flex-col flex-1 justify-between pl-5 min-w-0">
        <CardHeader className="px-0 pt-0 pb-0 gap-0.5">
          <CardTitle variant="default" size="default" className="text-[22px] font-bold leading-tight">
            {name}
          </CardTitle>
  ...
  <div className="flex flex-col gap-4">
    <span className="text-2xl font-bold text-right">${price.toLocaleString()}</span>
    <Button
      variant="secondary"
      size="default"
