...
import { OrderCardData, BundleCardData } from '@/shared/data/core/catalog/catalog.data';
...
export function OrderConfirmModal({ open, onOpenChange }: TOrderConfirmModalProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4 overflow-y-auto max-h-[460px] scrollbar-hide">
          {[...Array(5)].map((_, i) => (
            <OrderCard
              key={i}
        ...
        <div className="flex items-center justify-center gap-3 pt-2 border-t border-neutral-100">
          <Button
            variant="outline"
            className="rounded-full h-11 px-8 gap-2 border-red-200 text-red-500 hover:bg-red-50"
            onClick={() => onOpenChange(false)}
          >
            <X className="w-4 h-4" />
            Cancel order
          ...
          <Button
            variant="default"
            className="rounded-full h-11 px-8 gap-2 bg-green-50 text-green-600 hover:bg-green-100 border-0"
            onClick={() => onOpenChange(false)}
          >
