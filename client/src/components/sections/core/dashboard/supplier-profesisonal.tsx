...
const ICON_MAP: Record<string, React.ReactNode> = {
  ...
  OrganizationFigma: <ICONS.OrganizationFigma size={20} color="currentColor" className="text-muted-foreground" />,
};
    ...
    <div className="h-[calc(100vh-64px)] overflow-hidden flex flex-col">
      ...
      <div className="w-full px-10 py-4 flex flex-col lg:flex-row gap-[30px] flex-1 min-h-0 items-stretch">
        ...
        <div className="w-[600px] shrink-0 flex flex-col gap-4 min-h-0">
          ...
          <div
            className="rounded-2xl bg-white overflow-hidden flex flex-col shadow-[0_8px_40px_rgba(0,0,0,0.08)]"
            style={{ height: '300px' }}
          >
            <div className="flex items-center justify-between px-5 py-4 shrink-0">
              ...
              <Button
                variant="secondary"
                className="rounded-full bg-secondary/10 text-secondary hover:bg-secondary/20 border-0"
            ...
            <div className="grid grid-cols-[2fr_1fr_0.8fr_0.8fr_0.6fr] px-5 py-3 border-y border-neutral-50 shrink-0">
              {['PRODUCT NAME', 'CATEGORY', 'STOCK LEVEL', 'STATUS', 'ACTION'].map(col => (
                <span key={col} className="text-[12px] font-bold uppercase tracking-widest text-muted-foreground">
                  {col}
                </span>
              ))}
            </div>
            ...
            <div className="overflow-y-auto scrollbar-hide flex-1">
              {InventoryData.map((item, i) => (
                <div
                  key={i}
                  className="grid grid-cols-[2fr_1fr_0.8fr_0.8fr_0.6fr] items-center px-5 py-3 border-b border-neutral-50 last:border-0 hover:bg-neutral-50 transition-colors"
                >
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-lg bg-neutral-100 shrink-0" />
                    <span className="text-sm font-medium text-muted-foreground leading-tight">{item.name}</span>
                  </div>

                  <span className={`inline-flex w-fit px-2 py-0.5 rounded-full text-[12px] font-semibold ${CATEGORY_STYLES[item.category] ?? 'bg-neutral-100 text-neutral-600'}`}>
                    {item.category}
                  </span>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className={`text-sm ${INVENTORY_STATUS_STYLES[item.status]}`}>{item.status}</span>
                  </div>
          ...
          <div className="grid grid-cols-3 gap-4 shrink-0">
            {SupplierStatsData.map(stat => (
              <div
                key={stat.label}
                className="relative rounded-2xl border border-neutral-100 p-5 bg-white min-h-[130px] shadow-[0_8px_40px_rgba(0,0,0,0.08)]"
              >
                {stat.badge && (
                  <span className={`absolute top-2.5 right-2.5 text-[12px] font-semibold px-1.5 py-0.5 rounded-full ${stat.badgeColor}`}>
                    {stat.badge}
                  </span>
                )}
          ...
          <div className="rounded-2xl border border-neutral-100 bg-white p-5 flex items-center justify-between shrink-0 shadow-[0_8px_40px_rgba(0,0,0,0.08)]">
            <div>
              <p className="text-lg font-semibold">Profile Completeness</p>
              <p className="text-sm text-muted-foreground mt-0.5">Increase visibility by finishing your setup.</p>
            </div>
            <button className="w-11 h-11 rounded-full bg-foreground flex items-center justify-center shrink-0 hover:bg-foreground/80 transition-colors">
