    ...
    <div className="flex flex-col gap-5 px-10 py-10 bg-secondary/10">
      ...
      <Carousel opts={{ align: 'start', dragFree: true }} className="w-full">
        <CarouselContent className="-ml-5 border-0">
          {[...Array(6)].map((_, i) => (
            <CarouselItem key={i} className="pl-5 basis-[577px] border-0">
              <div
        ...
        <CarouselNext className="right-0 w-10 h-10 bg-white border border-neutral-200 hover:bg-neutral-50 shadow-sm" />
      </Carousel>
    </div>
  );
}