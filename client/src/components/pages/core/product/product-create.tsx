...
import { useGetCategories, useGetSpaces, useCreateProduct } from '@/hooks/queries/catalog.query';
...
export function ProductCreatePage() {
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      // Step 1: Create product first (backend will generate SKU)
      const productData = {
        title: data.title,
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        // Format: catalog/product/${sku}/${index}.png
        const imageUrl = `catalog/product/${sku}/${i}.png`;
        imageUrls.push(imageUrl);
      }

      // Step 3: Update product with image URLs
      // TODO: Add update product API call with images
  ...
  const toggleSpace = (spaceId: string) => {
    ...
    const newSpaces = current.includes(spaceId)
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                {spaces?.map(space => (
                  <Button
                    key={space.id}
                    type="button"
                    variant={selectedSpaces.includes(space.id) ? 'default' : 'outline'}
