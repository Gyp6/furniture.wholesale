...
export function OrderDetailPage({ params }: Props) {
        ...
        return (
          <div className={'flex items-center gap-3'}>
            <Button
              variant={'default'}
              className={
                'rounded-full h-11 gap-2 px-6 bg-green-600 hover:bg-green-700'
              }
              onClick={() =>
            ...
            <Button
              variant={'outline'}
              className={
                'rounded-full h-11 gap-2 px-6 text-red-600 border-red-200 hover:bg-red-50'
              }
              onClick={() =>
      ...
      if (orderData.status === 'PROCESSING') {
        return (
          <Button
            variant={'default'}
            className={
              'rounded-full h-11 gap-2 px-6 bg-blue-600 hover:bg-blue-700'
            }
            onClick={() =>
  ...
  if (isSubOrder) {
    ...
    const subTotal = items.reduce(
      (sum: number, item: any) => sum + item.quantity * item.priceSnapshot,
      0,
    );
      ...
      <div className={'w-full max-w-6xl mx-auto p-6 space-y-6'}>
        ...
        <div className={'bg-white rounded-[30px] p-8 shadow-md space-y-6'}>
          <div className={'flex items-start justify-between'}>
            ...
            <Badge
              className={`${statusConfig.color} flex items-center gap-2 px-4 py-2 text-base`}
            >
        ...
        <div className={'space-y-4'}>
          ...
          <Card
            className={
              'bg-white rounded-[30px] p-6 border-0 shadow-md space-y-3'
            }
          >
            {items.map((item: any) => {
              ...
              return (
                <div
                  key={item.id}
                  className={
                    'flex items-center gap-4 p-3 bg-neutral-50 rounded-[15px]'
                  }
                >
                  <div
                    className={
                      'relative w-16 h-16 rounded-[12px] overflow-hidden bg-white shrink-0'
                    }
                  >
                    <Image
                      src={imgSrc}
      ...
      <div className={'bg-white rounded-[30px] p-8 shadow-md space-y-6'}>
        ...
        <div className={'grid grid-cols-1 md:grid-cols-3 gap-6 pt-4'}>
          ...
          <Card className={'p-6 border-0 bg-neutral-50 rounded-[20px]'}>
            ...
            <p className={'text-2xl font-bold'}>
              $
              {order.platformFee.toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
            </p>
      ...
      <div className={'space-y-4'}>
        ...
        {order.subOrders.map((subOrder, idx) => {
          const subStatusConfig =
            ORDER_STATUS_CONFIG[
              subOrder.status as keyof typeof ORDER_STATUS_CONFIG
            ] || ORDER_STATUS_CONFIG.NEW;
          const SubStatusIcon = subStatusConfig.icon;
          const supplierName =
            subOrder.supplier?.profile?.companyName ||
          ...
          return (
            <Card
              key={subOrder.id}
              className={
                'bg-white rounded-[30px] p-6 border-0 shadow-md space-y-4'
              }
            >
              <div className={'flex items-start justify-between pb-4 border-b'}>
                <div className={'flex items-start gap-4'}>
                  <div
                    className={
                      'w-12 h-12 rounded-full bg-secondary/10 flex items-center justify-center shrink-0'
                    }
                  >
                  ...
                  <div>
                    ...
                    <p className={'text-sm font-medium mt-1'}>
                      Total: $
                      {totalSubOrderAmount.toLocaleString('en-US', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </p>
                ...
                <div className={'flex items-center gap-3'}>
                  ...
                  <Badge
                    className={`${subStatusConfig.color} flex items-center gap-1 px-3 py-1`}
                  >
