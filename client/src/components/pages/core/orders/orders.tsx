...
import { Package, ChevronRight, Clock, CheckCircle2, XCircle, AlertCircle, Truck } from 'lucide-react';
...
import { useGetMyOrders, useGetReceivedOrders, useUpdateOrderStatus } from '@/hooks/queries';
...
export function OrdersPage() {
  ...
  const { data: receivedOrders, isLoading: receivedLoading } = useGetReceivedOrders();
        ...
        <div className={"space-y-4"}>
          {orders.map(order => {
            ...
            const itemCount = isSubOrder
              ...
              : (order as any).subOrders?.reduce((s: number, so: any) => s + so.items.length, 0) ?? 0;
                  ...
                  <div className={"flex-1 space-y-3"}>
                    ...
                    <div className={"flex items-baseline gap-4"}>
                      ...
                      <div>
                        ...
                        <p className={"text-lg font-semibold"}>
                          ${((order as any).totalAmount ?? (order as any).items?.reduce((s: number, i: any) => s + i.quantity * i.priceSnapshot, 0) ?? 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </p>
                      </div>
                      <div>
                    ...
                    {!isSubOrder && (order as any).subOrders && (
                      <div className={"flex items-center gap-2"}>
                        <p className={"text-sm text-muted-foreground"}>
                          Split into {(order as any).subOrders.length} {(order as any).subOrders.length === 1 ? 'supplier' : 'suppliers'}
                        </p>
                        <div className={"flex gap-1"}>
                          {(order as any).subOrders.slice(0, 3).map((sub: any, idx: number) => (
                  ...
                  <div className={"flex items-center gap-2 shrink-0"}>
                    {isSubOrder && order.status === 'NEW' && (
                      <div className={"flex items-center gap-2"} onClick={e => e.stopPropagation()}>
                        ...
                        <Button
                          size={"sm"}
                          variant={"outline"}
                          className={"rounded-full h-8 gap-1 text-red-600 border-red-200 hover:bg-red-50 text-xs"}
                          onClick={() => {
                            toast.promise(
                              new Promise((resolve, reject) => {
                                updateStatus(
                    ...
                    {isSubOrder && order.status === 'PROCESSING' && (
                      <div onClick={e => e.stopPropagation()}>
                        <Button
                          size={"sm"}
                          className={"rounded-full h-8 gap-1 bg-blue-600 hover:bg-blue-700 text-xs"}
                          onClick={() => {
                            toast.promise(
                              new Promise((resolve, reject) => {
                                updateStatus(
